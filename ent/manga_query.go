...
func (mq *MangaQuery) QueryChapters() *ChapterQuery {
	query := &ChapterQuery{config: mq.config}
...
func (mq *MangaQuery) sqlAll(ctx context.Context) ([]*Manga, error) {
	...
	if query := mq.withChapters; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.Chapters = []*Chapter{}
		}
		query.withFKs = true
		...
		for _, n := range neighbors {
			fk := n.manga_chapters
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "manga_chapters" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "manga_chapters" returned %v for node %v`, *fk, n.ID)
			}
...
func (mq *MangaQuery) sqlExist(ctx context.Context) (bool, error) {
...
func (mgb *MangaGroupBy) sqlQuery() *sql.Selector {
