...
func (cq *ChapterQuery) QueryManga() *MangaQuery {
	query := (&MangaClient{config: cq.config}).Query()
...
func (cq *ChapterQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Chapter, error) {
	...
	if query := cq.withManga; query != nil {
		if err := cq.loadManga(ctx, query, nodes, nil,
			func(n *Chapter, e *Manga) { n.Edges.Manga = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *ChapterQuery) loadManga(ctx context.Context, query *MangaQuery, nodes []*Chapter, init func(*Chapter), assign func(*Chapter, *Manga)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "manga_chapters" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
